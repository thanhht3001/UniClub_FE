...
import CompetitionMoreHeader from './components/CompetitionMoreHeader';
import CompetitionMoreBody from './components/CompetitionMoreBody';
import { useHistory } from 'react-router';

function CompetitionMorePage(props) {
  ...
  async function loadDataListCompetition(currentPage, cases, search) {
    ...
    if (localStorage && localStorage.getItem('accessToken')) {
      ...
      let data = `clubId=${clubId}&name=${search}&pageSize=20&currentPage=${currentPage}${statusString}`;
      if (props.location.state.isEvent === 1) {
        data = `clubId=${clubId}&name=${search}&pageSize=20&currentPage=${currentPage}${statusString}&event=false`;
      }
      if (props.location.state.isEvent === 2) {
        data = `clubId=${clubId}&name=${search}&pageSize=20&currentPage=${currentPage}${statusString}&event=true`;
      }
    ...
    <>
      ...
      <div className="main-content" ref={mainContentRef}>
        ...
        {clubData && competitionList ? (
          <>
            <CompetitionMoreHeader clubData={clubData} />
            <CompetitionMoreBody
              competitionList={competitionList}
              loadDataListCompetition={loadDataListCompetition}
              checkCase={checkCase}
...
export default CompetitionMorePage;
